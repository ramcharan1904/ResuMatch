...
_VECTORS = {
    "Built ETL pipelines using Python and SQL for the analytics team.": [1.0, 0.0, 0.0],
    "Led a migration to a cloud-based data warehouse.": [0.0, 1.0, 0.0],
    "BS Computer Science": [0.0, 0.0, 1.0],
...
def test_finds_best_matching_bullet_for_each_keyword(monkeypatch):
    ...
    # Bullets and keywords must be merged into a single embeddings call, not two.
    assert len(calls) == 1
    assert calls[0] == [
        "Built ETL pipelines using Python and SQL for the analytics team.",
        "Led a migration to a cloud-based data warehouse.",
...
def test_keyword_below_threshold_is_omitted(monkeypatch):
    vectors = {
        "Built ETL pipelines using Python and SQL for the analytics team.": [1.0, 0.0, 0.0, 0.0],
        "Led a migration to a cloud-based data warehouse.": [0.0, 1.0, 0.0, 0.0],
        "BS Computer Science": [0.0, 0.0, 1.0, 0.0],
        "Kubernetes": [0.0, 0.0, 0.0, 1.0],  # orthogonal to every bullet
    }
